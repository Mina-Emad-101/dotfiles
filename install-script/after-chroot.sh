...
fdisk -l;
read -p "Enter 'sd(nx)' for boot partition: " boot_partition;
mount "/dev/$boot_partition" /boot;
grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=GRUB;
grub-mkconfig -o /boot/grub/grub.cfg;
sed -i "s/#GRUB_DISABLE_OS/GRUB_DISABLE_OS/" /etc/default/grub;
fdisk -l;
read -p "Enter 'sd(nx)' for Windows boot partition (default: empty): " win_boot_partition;
if [[ -n $win_boot_partition ]]; then
	echo $win_boot_partition;
